namespace PrimitiveSurvival.ModSystem
{
    ...
    public class BESupport : BlockEntityDisplayCase, ITexPositionSource
    {
        ...
        private void CheckForWaterSource(char side, string supportDir)
        {
            ...
            var neibPos1 = this.tmpNeibPos1; switch (side)
            {
                case 'e':
                    {
                        if (supportDir == "ns")
        ...
        private bool TryTake(IPlayer byPlayer, ItemSlot sourceSlot)
        {
            if (!sourceSlot.Empty)
            {
                ...
                if (item != null)
                {
                    var tempStack = new ItemStack(item, sourceSlot.StackSize); var takeOK = byPlayer.InventoryManager.TryGiveItemstack(tempStack);
