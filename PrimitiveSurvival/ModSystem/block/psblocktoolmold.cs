...
#nullable disable
    ...
    // temp fix - remove once this is fixed in vanilla blocktoolmold
    public class PSBlockToolMold : Block
    {
            ...
            interactions = ObjectCacheUtil.GetOrCreate(api, Variant["tooltype"] + "moldBlockInteractions", () =>
            {
                ...
                return new WorldInteraction[] {
                    new WorldInteraction()
                    {
                        ...
                        {
                            PSBlockEntityToolMold betm = api.World.BlockAccessor.GetBlockEntity(bs.Position) as PSBlockEntityToolMold;
        ...
        {
            if (world.Rand.NextDouble() < 0.05 && GetBlockEntity<PSBlockEntityToolMold>(pos)?.Temperature > 300)
            {
        ...
        public override float GetTraversalCost(BlockPos pos, EnumAICreatureType creatureType)
        {
            ...
            {
                if (GetBlockEntity<PSBlockEntityToolMold>(pos)?.Temperature > 300) return 10000f;
            }
        ...
        {
            var boxesRotated = getColSelRotatedBoxes(SelectionBoxes, (world.GetBlockEntity(pos) as PSBlockEntityToolMold)?.MeshAngle ?? 0);
        ...
        {
            return getColSelRotatedBoxes(CollisionBoxes, (blockAccessor.GetBlockEntity(pos) as PSBlockEntityToolMold)?.MeshAngle ?? 0);
        }
        ...
        public override void OnHeldInteractStart(ItemSlot itemslot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel, bool firstEvent, ref EnumHandHandling handHandling)
        {
            ...
            if (byEntity.World.BlockAccessor.GetBlockEntity(blockSel.Position.AddCopy(blockSel.Face.Opposite)) is PSBlockEntityToolMold betm)
            {
        ...
        {
            if (world.BlockAccessor.GetBlockEntity(blockSel?.Position) is PSBlockEntityToolMold betm)
            {
        ...
        public override bool DoPlaceBlock(IWorldAccessor world, IPlayer byPlayer, BlockSelection blockSel, ItemStack byItemStack)
        {
            ...
            if (val && world.BlockAccessor.GetBlockEntity(blockSel.Position) is PSBlockEntityToolMold betm)
            {
        ...
        {
            if (world.BlockAccessor.GetBlockEntity(pos) is PSBlockEntityToolMold betm)
            {
        ...
        {
            if (world.BlockAccessor.GetBlockEntity(pos) is PSBlockEntityToolMold betm && betm.Shattered)
            {
