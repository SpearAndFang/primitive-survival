...
#nullable disable
    ...
    //temp fix - remove once this is fixed in vanilla BlockEntityToolMold
    public class PSBlockEntityToolMold : BlockEntity, ILiquidMetalSink, ITemperatureSensitive, ITexPositionSource, IRotatable
    {
        protected PSToolMoldRenderer renderer;
        ...
        public override void Initialize(ICoreAPI api)
        {
            ...
            {
                capi.Event.RegisterRenderer(renderer = new PSToolMoldRenderer(this, capi, fillQuadsByLevel), EnumRenderStage.Opaque, "pstoolmoldrenderer");
                UpdateRenderer();
        ...
        public bool OnPlayerInteract(IPlayer byPlayer, BlockFacing onFace, Vec3d hitPosition)
        {
            ...
            if (!sneaking)
            {
                ...
                if (!handled && FillLevel == 0)
                {
                    ...
                    if (activeStack != null && activeStack.Collectible is not PSBlockToolMold and not BlockIngotMold) return handled;
        ...
        private void GenMeshes()
        {
            MoldMesh = ObjectCacheUtil.GetOrCreate(Api, Block.Variant["color"] + Block.Variant["tooltype"] + "toolmold", () =>
            {
                ...
                Shape shape = Shape.TryGet(Api, "primitivesurvival:shapes/block/clay/mold/" + Block.Variant["tooltype"] + ".json");
